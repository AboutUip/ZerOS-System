...
class ThemeManager {
    ...
    static async _applyDesktopBackground(backgroundId) {
        ...
        if (isLocalPath) {
            ...
            // 处理相对路径（系统资源路径）
            // 例如: assets/desktopBG/default.svg -> /system/assets/assets/desktopBG/default.svg
            if (background.path.startsWith('assets/')) {
                imageUrl = '/system/assets/' + background.path;
            } else if (!background.path.startsWith('/') && !background.path.startsWith('http://') && !background.path.startsWith('https://')) {
                // 其他相对路径，添加 /system/assets/ 前缀
                imageUrl = '/system/assets/' + background.path;
            } else if (background.path.startsWith('/')) {
                // 已经是绝对路径，直接使用
                imageUrl = background.path;
            }
            
            // 确保 imageUrl 是绝对路径（以 / 开头）
            if (imageUrl && !imageUrl.startsWith('/') && !imageUrl.startsWith('http://') && !imageUrl.startsWith('https://')) {
                imageUrl = '/system/assets/' + imageUrl;
            }
            
            // 调试日志
            KernelLogger.debug("ThemeManager", `桌面背景路径转换: ${background.path} -> ${imageUrl}`);
        }
        ...
        if (isVideo) {
            ...
            // 确保使用绝对路径
            const absoluteImageUrl = imageUrl.startsWith('/') || imageUrl.startsWith('http://') || imageUrl.startsWith('https://')
                ? imageUrl
                : '/' + imageUrl;
            video.src = absoluteImageUrl;
            video.autoplay = true;
            video.loop = true;
            ...
            if (root) {
                ...
                root.style.setProperty('--desktop-background-video-url', `url('${absoluteImageUrl}')`);
            }
            ...
            // 应用背景图
            // 确保使用绝对路径，避免浏览器基于当前页面路径解析
            const absoluteImageUrl = imageUrl.startsWith('/') || imageUrl.startsWith('http://') || imageUrl.startsWith('https://')
                ? imageUrl
                : '/' + imageUrl;
            guiContainer.style.backgroundImage = `url('${absoluteImageUrl}')`;
            KernelLogger.debug("ThemeManager", `设置桌面背景图片: ${absoluteImageUrl}`);
            ...
            if (root) {
                const absoluteImageUrl = imageUrl.startsWith('/') || imageUrl.startsWith('http://') || imageUrl.startsWith('https://')
                    ? imageUrl
                    : '/' + imageUrl;
                root.style.setProperty('--desktop-background-image', `url('${absoluteImageUrl}')`);
                // 添加图片类型标记（供 CSS 使用）
                if (isGif) {
    ...
    static getSystemIconPath(iconName, styleId = null) {
        ...
        // 图标路径：system/assets/assets/icons/{styleId}/{iconName}.svg
        return `system/assets/assets/icons/${currentStyleId}/${iconName}.svg`;
    }
...
if (typeof DependencyConfig !== 'undefined') {
    DependencyConfig.publishSignal("../system/ui/themeManager.js");
}
