...
class GUIManager {
    ...
    static focusWindow(windowIdOrPid) {
        ...
        if (windowInfo.window && windowInfo.window.style) {
            ...
            // 使用 requestAnimationFrame 和 setTimeout 确保浏览器已经应用了样式
            requestAnimationFrame(() => {
                setTimeout(() => {
                    ...
                    if (focusedComputedZIndex < maxComputedZIndex && maxComputedWindowId !== windowId) {
                        ...
                        // 只在调试模式下记录详细警告，正常情况只记录修复信息
                        if (KernelLogger && typeof KernelLogger.debug === 'function') {
                            KernelLogger.debug("GUIManager", `已修复焦点窗口 z-index: ${windowId} 从 ${focusedComputedZIndex} 设置为 ${fixedZIndex}`);
                        }
                    } else {
                        KernelLogger.debug("GUIManager", `焦点窗口 z-index 验证通过: ${windowId} = ${focusedComputedZIndex}, 所有窗口: ${JSON.stringify(allComputedZIndexes)}`);
                    }
                }, 10); // 增加延迟，确保浏览器有足够时间应用样式
            });
...
if (typeof DependencyConfig !== 'undefined' && DependencyConfig && typeof DependencyConfig.publishSignal === 'function') {
    DependencyConfig.publishSignal("../system/ui/guiManager.js");
} else if (typeof document !== 'undefined' && document.body) {
    // 降级方案：直接发布事件
    document.body.dispatchEvent(
        new CustomEvent("dependencyLoaded", {
            detail: {
                name: "../system/ui/guiManager.js",
            },
        })
    );
    if (typeof KernelLogger !== 'undefined') {
