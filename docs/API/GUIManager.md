...
7. **弹性内容窗口**: 包含 iframe 的窗口会被自动识别为弹性内容窗口，初始尺寸固定为 600x450 像素，最小尺寸限制为 600x450 像素（不允许用户缩小到小于初始尺寸），但可以拉伸到更大尺寸。普通应用（如扫雷等使用 flex 布局但不含 iframe）不受此限制。

## 弹性内容窗口处理

GUIManager 自动检测窗口是否包含弹性内容（目前仅检测 iframe 元素），并为这类窗口提供特殊的尺寸处理：

### 检测机制
- **检测对象**: 仅检测窗口内的 iframe 元素
- **检测结果**: 包含 iframe 的窗口会被标记为弹性内容窗口

### 尺寸处理
- **初始尺寸**: 600 x 450 像素
- **最小尺寸**: 600 x 450 像素（不允许用户缩小到小于此尺寸）
- **最大尺寸**: 无限制（用户可以拉伸到任意大小）

### 与普通窗口的区别
| 特性 | 普通窗口 | 弹性内容窗口（含 iframe） |
|------|----------|--------------------------|
| 初始尺寸 | 程序自定义或居中 | 固定 600x450 |
| 最小尺寸 | 300x200 | 600x450 |
| 最大尺寸 | 无限制 | 无限制 |

### 开发者说明
- 如果应用不想受到弹性内容窗口的限制，应避免在窗口内使用 iframe
- 如果需要使用 iframe 但希望自定义初始尺寸，可以在注册窗口前手动设置 `windowElement.style.width` 和 `windowElement.style.height`
