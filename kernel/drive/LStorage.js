...
class LStorage {
    ...
    static async getSystemStorage(key) {
        ...
        if (isSensitiveKey) {
            ...
            } else if (key.startsWith('exceptionHandler.') && isSystemLoading) {
                // 系统加载期间，ExceptionHandler 模块需要访问其自己的键（用于初始化）
                // 检查调用栈是否包含 exceptionHandler
                const hasExceptionHandlerPath = /kernel[\/\\]core[\/\\]exceptionHM[\/\\]/i.test(fullCallStack);
                const hasExceptionHandlerName = /exceptionHandler/i.test(fullCallStack);
                if (hasExceptionHandlerPath || hasExceptionHandlerName) {
                    KernelLogger.debug("LStorage", `系统加载中，ExceptionHandler 模块访问其自己的键 ${key}，允许读取`);
                    // 允许访问，直接返回（跳过后续的 PID 检查）
                    try {
                        const value = LStorage._storageData.system[key] ?? null;
                        return value;
                    } catch (error) {
